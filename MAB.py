...
from tracker import TrackerSWTS, TrackerREXP3, TrackerLM,TrackerSWTS1
...
class GenericMAB:
    ...
    def D_TS_gaussian(self, T, gamma, mu_0, sigma_0, sigma, store_rewards_arm=False):
        """
    ...
    def TS_gaussian(self, T, store_rewards_arm=False):
        """
        ...
        for t in range(T):
            ...
            sigma=5/(tr.nb_draws+1)
            tr.update(t, arm, reward)
        return tr
    
    def DS_TS_gaussian(self, T, gamma,sigma=1/2, store_rewards_arm=False):
        """
        Our method
        :param T:  time horizon.
        :param gamma:  discount factor used to compute the posterior
        ...
        :return:
        """
        tr = TrackerDTS1(self.means, T, gamma, store_rewards_arm=store_rewards_arm)
        ...
        for t in range(T):
            ...
            tr.update(t, arm, reward)
            for i in range(K):
                if  tr.nb_draws[i]<1e-8:
                    tao[i]=10000
                else:
                    tao[i]=sigma/(tr.nb_draws[i])**0.5
                if tr.nb_draws[i]<1e-8:
                    mu_hat[i]=tr.S[i]
                else:
    ...
    def SW_TS(self, T, tau, store_rewards_arm=False):
        """
    ...
    def SW_TS_gaussian(self, T, tau, sigma, store_rewards_arm=False):
        """
        SW_TS for Gaussian distributions
        :param T: T: T: time horizon.
        :param tau: Length of the sliding window used to compute the posterior
        :param sigma: Known variance (or upper-bound) for the different arms
        :param store_rewards_arm: Storing the rewards for the different arms.
    ...
    def SW_TS_gaussian_sta(self, T, tau, mu_0, sigma_0, sigma, store_rewards_arm=False):
        """
        ...
        :return:
        """
        tr = TrackerSWTS1(self.means, T, tau, store_rewards_arm=store_rewards_arm)
        K=tr.nb_draws.shape[0]
        tao=np.ones(K)
        mu_hat=np.zeros(K)
        for t in range(T):
            ...
            for i in range(K):
                if tr.nb_draws[i]==0:
                    ...
                    tao[i]=1/(tr.nb_draws[i])**0.5#min(1/(tr.nb_draws[i])**0.5,1000)
    ...
    def LB_SDA(self, T, tau=None, explo_func=default_exp, diversity_func=default_diversity):
        ...
        :param T: time horizon.
        :param tau: length of the sliding window.
        :param explo_func: Exploration function used.
