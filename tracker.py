...
class TrackerDTS(Tracker2):
    def __init__(self, means, T, gamma, store_rewards_arm=False):
        ...
        self.t = 0
        self.mu_hat=np.ones(self.nb_arms)/2
    ...
    def update(self, t, arm, reward):
        ...
        self.F[arm] += 1-reward
        self.nb_draws[arm] += 1
        self.mu_hat[arm]=self.S[arm]/self.nb_draws[arm]
...
class TrackerDTS1(Tracker2):
    def __init__(self, means, T, gamma, store_rewards_arm=False):
        super(TrackerDTS1, self).__init__(means, T, store_rewards_arm)
        self.gamma = gamma
        self.time_changes = [0]
        self.S = np.zeros(self.nb_arms)
        #self.F = np.zeros(self.nb_arms)
        self.nb_draws = np.zeros(self.nb_arms)
        self.t = 0
        self.Naa=np.zeros(self.nb_arms)
...
class TrackerSWTS1(Tracker2):
    def __init__(self, means, T, tau, store_rewards_arm=False):
        super(TrackerSWTS1, self).__init__(means, T, store_rewards_arm)
        self.tau = tau
        self.time_changes = [0]
        self.S = np.zeros(self.nb_arms)
        self.F = np.zeros(self.nb_arms)
        self.nb_draws = np.zeros(self.nb_arms)
        self.t = 0
        self.tau0=1#np.ones(self.nb_arms)*10#int(np.sqrt(tau))
        self.Na=np.zeros(self.nb_arms)
        self.flag=np.zeros(self.nb_arms)
    ...
    def update(self, t, arm, reward):
        ...
        self.Na[arm] += 1
        self.flag[arm]=1
        if np.sum(self.flag)==self.nb_arms:
            self.tau0 = min(self.T,self.tau0*np.exp(1/np.log(self.T)))
            self.flag=np.zeros(self.nb_arms)
        if t >= int(self.tau0):
            first_t = int(t - int(self.tau0))
            first_element = int(self.arm_sequence[first_t])
            self.S[first_element] -= self.reward[first_t]
            self.F[first_element] -= (1-self.reward[first_t])
            self.nb_draws[first_element] -= 1
        ...
        #     if t > int(self.tau0[i]):
        #         first_t = int(t - int(self.tau0[i]))
        #         first_element = int(self.arm_sequence[first_t])
        #         if first_element==i:
        #             self.S[first_element] -= self.reward[first_t]
        #             self.F[first_element] -= (1-self.reward[first_t])
        #             #if self.nb_draws[first_element]!=0:
        #             self.nb_draws[first_element] -= 1
